...
# ╔═╡ e024dc5e-7b7d-47ae-919c-3bade5704caf
begin
	using LinearAlgebra
...
# ╔═╡ ff39d2e6-6949-433b-9982-7c09ec956005
begin
	L  = 0.10
	ω  = 2.0
	θ1 = π/4
...
# ╔═╡ 3ac0bcef-a581-4b46-8f19-7256a9f7af0d
begin
	# q = [s1, s2, x3, y3, θ3]
	C(q, t) = [
		q[3] - (L/2)*cos(q[5]) - q[1]*cos(θ1),
		q[4] - (L/2)*sin(q[5]) - q[1]*sin(θ1),
		q[3] + (L/2)*cos(q[5]) - q[2]*cos(θ2),
		q[4] + (L/2)*sin(q[5]) - q[2]*sin(θ2),
		q[5] - ω*t
	]
end

# ╔═╡ b8f63580-8373-45e4-a55e-7719b718c5d8
begin
	dCdq(q, t) = [
		-cos(θ1)   0           1   0   (L/2)*sin(q[5]);
		-sin(θ1)   0           0   1  -(L/2)*cos(q[5]);
		 0        -cos(θ2)    1   0  -(L/2)*sin(q[5]);
		 0        -sin(θ2)    0   1   (L/2)*cos(q[5]);
		 0         0          0   0   1
	]
...
# ╔═╡ 9dbafc21-d2ce-4538-b0bb-2379d771f00b
begin
	time = LinRange(0, 2π/ω, 200)
	q_all = zeros(length(time), 5)

	q_guess = [-0.07, 0.07, 0.0, -0.05, 0.0]

	for i in eachindex(time)
		t = time[i]
		prob = NonlinearProblem(C, q_guess, t)
		sol = solve(prob)
		q_all[i, :] = sol.u
		q_guess = sol.u
	end
...
# ╔═╡ e07daaee-a8c4-4c01-a491-9180a0aa32f8
begin
	dq_all = zeros(length(time), 5)

	for i in eachindex(time)
		q = q_all[i, :]
		t = time[i]
		dq_all[i, :] = -(dCdq(q, t) \ Ct(q, t))
	end
end

# ╔═╡ aabb694f-1cd3-49fa-9015-bc370a0e2abd
begin
	ddq_all = zeros(length(time), 5)
	...
	for i in eachindex(time)
		...
		ddq_all[i, :] = dCdq(q, t) \ gamma
	end
end

# ╔═╡ 82140511-a0b1-4642-bb6b-ea1ec72a5ac2
begin
	plot(time, dq_all, label=["s1̇" "s2̇" "ẋ3" "ẏ3" "θ̇3"],
	     xlabel="t [s]", ylabel="velocity", title="Generalized Velocities")
end

# ╔═╡ 8e34cf38-5460-448a-8816-c8843bfee465
begin
	plot(time, ddq_all, label=["s1̈" "s2̈" "ẍ3" "ÿ3" "θ̈3"],
	     xlabel="t [s]", ylabel="acceleration", title="Generalized Accelerations")
end

# ╔═╡ 8c661f47-9c6a-410d-844f-028db50310c6
...
manifest_format = "2.0"
project_hash = "f85761575d71cab9a946ac4aee13ca67e18d8e6a"
